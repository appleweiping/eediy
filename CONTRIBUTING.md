...
Write the learner-facing article around the course, not around EEDIY's review
workflow. Evidence level, review date, correction forms, and the full resource
inventory are rendered separately. Do not repeat `R0`, maintainer notes,
ledgers, dossiers, or submission instructions in the article, and do not force
every course into the same diagnostic-and-acceptance skeleton.
...
课程候选 + 方向图 + 官方资源抓取证据
  ...
  + bilingual content/track-guides fragments
...
`data/course_resources.json`. Editorial records then add the bilingual summary
and review note to `data/courses.json`; canonical courses, `data/routes.json`, the
researched-guide manifest, bilingual course fragments, and authored bilingual
track fragments feed the page generator.
...
`data/course_resources.json`；编辑记录随后把双语摘要与复核说明叠加到
`data/courses.json`。
权威课程目录、`data/routes.json`、导读清单、双语单课导读片段和
`content/track-guides/` 下的双语方向导读共同驱动页面生成。
...
`run_quality.py` is the offline contribution gate. It checks upstream source
drift and dependency consistency, runs unit tests and the quality report, and
performs a strict site build while explicitly omitting the network resource
sweep. It writes disposable, ignored artifacts under `build/` and `site/`; it
does not rewrite authoritative source data. Do not delete generated data to
bypass drift checks.

`run_quality.py` 是离线贡献门禁：它检查上游源码漂移与依赖一致性，运行单元测试、
生成质量报告并严格构建站点，同时明确跳过全站网络资源扫描。它会在被忽略的
`build/` 与 `site/` 中写入可丢弃产物，但不会改写权威源数据。不得通过删除生成
数据来绕过漂移检查。

## Required verification / 必需验证

When changing URLs or resource metadata, also run the command below. The same
command is mandatory for every release, even when the current contribution did
not edit a URL:
...
The `--external` mode requires a fresh link report and treats every warning as
a release failure.

修改 URL 或资源元数据时还要运行下方命令；每次正式发布也都必须运行，即使当前
贡献没有修改 URL。`--external` 模式要求当次链接报告，并把所有警告作为发布失败。
...
resource coverage, access and licence conditions, verification date, tooling,
hardware, cost, and safety, plus a course-specific learning exit where one is
justified. Record provider workload only with a traceable provider source.
Otherwise label it as an estimate, explain the basis, and tell learners to time
a representative unit before committing to a long-term schedule.

课程贡献必须记录先修、产出、角色与分层、资源覆盖、访问和许可条件、核验日期，
并说明工具、硬件、成本与安全信息；确有必要时，再给出课程特有的学习出口。
只有在提供方来源可追溯时，才记录为提供方工作量；否则须明确标注为估算、解释
依据，并提示学习者先记录一个代表性单元的实际投入，再承诺长期进度。

Coverage scores are internal editorial signals, not reader-facing claims of
completeness: `0` means no usable set was verified in the review, `1` means the
set is partial, version-mismatched, or access-restricted, and `2` means a
coherent course-level material set is usable under its separately recorded
access conditions. A `2` never implies every term or file, public solutions,
grading, starter code, or feedback. In particular, `code: 2` requires a
reviewed repository, machine-readable file, or course-specific code index.

覆盖分数是内部编辑信号，不是面向读者的“完整”承诺：`0` 表示本次未核到可用集合，
`1` 表示材料局部、跨版本或受访问限制，`2` 表示在另行记录的访问条件下存在连贯的
课程级材料集合。`2` 不代表逐学期逐文件齐全，也不默认包含答案、评分、starter code
或反馈。尤其是 `code: 2`，必须有已复核的仓库、机器可读文件或课程专用代码索引。

Automated discovery lives in `data/course_resources.json`. Human-reviewed
additions and corrections belong in `data/course_resource_overrides.json`;
do not force a crawler heuristic to misclassify a video, product page, or PDF
just to raise a coverage score.

自动发现结果位于 `data/course_resources.json`；人工复核后的补充和纠错应写入
`data/course_resource_overrides.json`。不得为了提高覆盖分数，强行把视频、课程
产品页或 PDF 误判成代码。
