---
title: "Semiconductor Devices"
description: "Carriers, junctions, MOS structures, and nanostructures connecting quantum and solid-state physics to measurable devices."
page_type: track
---

<!-- generated-by: scripts/generate_course_pages.py; fingerprint: 82a3ce318af1a885 -->

# Semiconductor Devices

## Track position

Carriers, junctions, MOS structures, and nanostructures connecting quantum and solid-state physics to measurable devices.

## Recommended prerequisite tracks

- [Physics Foundations](../physics/index.md)
- [Circuit Analysis](../circuits/index.md)
- [Engineering Mathematics](../mathematics/index.md)

## Suggested order

1. [Physics of Semiconductors and Nanostructures](124-ece-4070.md)

## Courses

| Course | Institution | Role | Tier | Practice coverage |
|---|---|---|---|---|
| [Physics of Semiconductors and Nanostructures](124-ece-4070.md) | Cornell University | Mainline | A | Complete |
| [Introduction to Nanoelectronics](125-6-701.md) | MIT | Alternative | A | Partial |

## How to choose

- For a first systematic pass, start with audit-passed mainline courses; read the limitation before using a record marked “Audit review,” and rarely take parallel alternatives.
- Tiers measure public-resource completeness and self-study executability, not institutional or instructor prestige.
- Use supplements only to close a specific topic, tool, or practice gap.

## Track completion

- Explain the core concepts, models, and methods of Semiconductor Devices
- Produce reproducible exercises, experiments, or designs with explicit checks
