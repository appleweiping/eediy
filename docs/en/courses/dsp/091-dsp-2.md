---
title: "Digital Signal Processing 2: Filtering"
description: "EPFL's Digital Signal Processing 2: Filtering develops filtering from first principles through videos, notebooks, exercises, and code, recommending basic calculus and linear algebra while potentially requiring paid platform access."
page_type: course
course_id: "course-091"
editorial_status: "catalogue"
evidence_level: "R0"
comments: true
---

<!-- generated-by: scripts/generate_course_pages.py; fingerprint: fa853f84f41b46b1 -->

# Digital Signal Processing 2: Filtering

## Course Overview

- **University:** EPFL
- **Course code:** DSP 2
- **Prerequisites:** Recommended foundation: Signals and Systems; Recommended foundation: Probability, Statistics, and Random Processes; Recommended foundation: Programming and Engineering Computing; Recommended background: Basic calculus and linear algebra; following the specialization numbering is recommended, not required
- **Track:** [Digital Signal Processing](index.md)
- **Path role:** Alternative
- **Public materials:** Core materials available
- **Last reviewed:** 2026-07-28

> **Resource catalogue:** This page confirms the course identity, official entry points, and public materials. The assignments have not yet been reviewed one by one, and this is not a completion report; use it to find the course, not as a stand-alone enrollment decision.

EPFL's Digital Signal Processing 2: Filtering develops filtering from first principles through videos, notebooks, exercises, and code, recommending basic calculus and linear algebra while potentially requiring paid platform access.

**Check before starting**

- Recommended foundation: Signals and Systems
- Recommended foundation: Probability, Statistics, and Random Processes
- Recommended foundation: Programming and Engineering Computing
- Recommended background: Basic calculus and linear algebra; following the specialization numbering is recommended, not required

## Start with these links

Use these entry points to decide whether the course fits. Per-lecture files and historical exams are kept in the complete index at the end of the page.

- [Course home](https://www.coursera.org/learn/dsp2)

## Known Boundaries

The provider recommends basic calculus and linear algebra but does not list the first DSP course as a hard prerequisite; full Coursera access may require payment.

This catalogue record does not present a maintainer-invented project, uniform workload, or generic acceptance test as a course fact. If you completed the course, use the discussion below to report assignment structure, actual effort, broken access, and concrete pitfalls.

## Course Resources

<details markdown="1">
<summary>Expand the complete resource index (1 items)</summary>

### Material coverage

| Type | Completeness |
|---|---|
| Video | Complete |
| Notes | Partial |
| Practice | Complete |
| Labs | Complete |
| Exams | No public material |
| Code | Complete |

### Resource

| Resource | Access | Status | Verified |
|---|---|---|---|
| [Course home](https://www.coursera.org/learn/dsp2) | Registration required | Listed by official page | 2026-07-28 |

> Links were discovered from official sources on the recorded date. Access does not grant redistribution rights, and region, account, third-party rights, or later redesigns may change availability.

</details>
