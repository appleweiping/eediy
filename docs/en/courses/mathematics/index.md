---
title: "Engineering Mathematics"
description: "Calculus, linear algebra, differential equations, complex variables, and numerical reasoning for modeling across EE."
page_type: track
---

<!-- generated-by: scripts/generate_course_pages.py; fingerprint: 7a497e79aeca4494 -->

# Engineering Mathematics

## Track position

Calculus, linear algebra, differential equations, complex variables, and numerical reasoning for modeling across EE.

## Recommended prerequisite tracks

- None

## Suggested order

1. [Single Variable Calculus](001-18-01sc.md)
2. [Multivariable Calculus](002-18-02sc.md)
3. [Differential Equations](003-18-03sc.md)
4. [Linear Algebra](004-18-06sc.md)

## Courses

| Course | Institution | Role | Tier | Practice coverage |
|---|---|---|---|---|
| [Single Variable Calculus](001-18-01sc.md) | MIT | Mainline | S | Complete |
| [Multivariable Calculus](002-18-02sc.md) | MIT | Mainline | S | Complete |
| [Differential Equations](003-18-03sc.md) | MIT | Mainline | S | Complete |
| [Linear Algebra](004-18-06sc.md) | MIT | Mainline | S | Complete |
| [Complex Variables with Applications](005-18-04.md) | MIT | Alternative | A | Complete |
| [Matrix Methods in Data Analysis, Signal Processing, and Machine Learning](009-18-065.md) | MIT | Alternative | A | Partial |
| [The Art of Approximation in Science and Engineering](018-6-055j.md) | MIT | Supplement | A | Complete |

## How to choose

- For a first systematic pass, start with audit-passed mainline courses; read the limitation before using a record marked “Audit review,” and rarely take parallel alternatives.
- Tiers measure public-resource completeness and self-study executability, not institutional or instructor prestige.
- Use supplements only to close a specific topic, tool, or practice gap.

## Track completion

- Explain the core concepts, models, and methods of Engineering Mathematics
- Produce reproducible exercises, experiments, or designs with explicit checks
